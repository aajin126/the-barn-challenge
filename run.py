...
def get_odom_coor(topic=ODOM_TOPIC):
    odom_msg = rospy.wait_for_message(topic, Odometry, timeout=5)
...
if __name__ == "__main__":
    ...
    launch_file = join(base_path, '..', 'jackal_helper/launch/move_base_EWHA.launch')
    ...
    goal_coor = (mb_goal.target_pose.pose.position.x, mb_goal.target_pose.pose.position.y)
    ...
    while compute_distance(goal_coor, curr_coor_odom) > 1 and not collided and curr_time - start_time < 25:
        curr_time = rospy.get_time()
        pos = gazebo_sim.get_model_state().pose.position
        curr_coor = (pos.x, pos.y)
        curr_coor_odom = get_odom_coor()
        print("distance to goal: %.2f (m)" %compute_distance(goal_coor, curr_coor_odom), end=", ")
    ...
    dist_to_goal = compute_distance(goal_coor, curr_coor_odom)
    print("distance to goal: %.2f (m)" %dist_to_goal)
    print("Navigation metric: %.4f" %(nav_metric))
    
    with open(args.out, "a") as f:
        f.write("%d %d %d %d %.4f %.4f %.2f\n" %(args.world_idx, success, collided, (curr_time - start_time)>=25, curr_time - start_time, nav_metric, dist_to_goal))
